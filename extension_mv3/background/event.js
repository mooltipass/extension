...
mooltipassEvent.onLoadSettings = async function (callback, tab) {
	//old before MV3
    //page.settings = (typeof (localStorage.settings) == 'undefined') ? {} : JSON.parse(localStorage.settings);
	
	try {			
		let storeItems = await chrome.storage.local.get(["settings"]);
				
		if (storeItems.settings){
			page.settings = JSON.parse(storeItems.settings)
		} else {
            page.settings = {};
        }				
    ...
    mooltipass.backend.loadSettings();  //loading blacklist from settings
}
...
mooltipassEvent.onGetSettings = function (callback, tab) {
    ...
    settings['extension-base'] = chrome.runtime.getURL('/');
...
mooltipassEvent.onSaveSettings = function (callback, tab, settings) {
	...
	chrome.storage.local.set(JSON.stringify(settings)).then(mooltipassEvent.onLoadSettings, function(){});
}
...
mooltipassEvent.showApp = function () {
    ...
    if (mooltipass.device.isConnectedToExternalApp()) {
		...
		chrome.runtime.sendMessage(mooltipass.device._app.id, { 'show': true });
    }
}
